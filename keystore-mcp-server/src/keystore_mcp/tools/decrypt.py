...
def register_decrypt_tools(server: Server) -> None:
    ...
    async def change_keystore_password(
        ...
        decrypt_result = await decrypt_keystore(keystore, old_password)
        
        if decrypt_result.get("error"):
            return decrypt_result
        
        private_key = decrypt_result["private_key"]
        old_kdf = decrypt_result["kdf_used"]
        
        # Determine new KDF
        if new_kdf:
            new_kdf = new_kdf.lower()
            if new_kdf not in ("scrypt", "pbkdf2"):
                return {
                    "error": True,
                    "code": "INVALID_KDF",
                    "message": f"Invalid KDF: {new_kdf}"
                }
        else:
            new_kdf = old_kdf
        ...
        from mcp.server import Server as TempServer
        
        # Determine parameters
        work_factor = 18 if upgrade_security else None
        iterations = 262144 if upgrade_security else None
        ...
        try:
            ...
            # Derive new key
            if new_kdf == "scrypt":
                from ..crypto.kdf import derive_key_scrypt, SCRYPT_N_STANDARD, SCRYPT_R, SCRYPT_P, SCRYPT_DKLEN
                ...
                from ..crypto.kdf import derive_key_pbkdf2, PBKDF2_ITERATIONS_STANDARD, PBKDF2_DKLEN
            ...
            security_upgraded = upgrade_security or (old_kdf == "pbkdf2" and new_kdf == "scrypt")
            
            return {
                "new_keystore": new_keystore,
